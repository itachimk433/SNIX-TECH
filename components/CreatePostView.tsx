...
export default function CreatePostView({ onSuccess, isGuest, onSignInRequired }: CreatePostViewProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (sharingMode === 'cloud_link') {
      if (!description.trim()) { setError("Please paste the cloud link URL."); return; }
      const hasUrl = /https?:\/\/[^\s]+/.test(description.trim());
      if (!hasUrl) { setError("Cloud link must be a valid URL starting with https:// or http://"); return; }
    } else {
    ...
    try {
      ...
      const postData: Record<string, unknown> = {
        ...
        authorAvatar: `https://api.dicebear.com/7.x/bottts/svg?seed=${encodeURIComponent(currentUser.displayName || "anon")}`,
        title: title.trim(),
      ...
      onSuccess();
    } catch (err: any) { setError("Failed to post: " + err.message); }
      ...
      <form onSubmit={handleSubmit} className="space-y-4 pb-20">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <button type="button" onClick={() => setSharingMode('downloadable')}
            ...
            <button type="button" onClick={() => setSharingMode('cloud_link')}
        ...
        <div>
          <VKInput
            label={sharingMode === 'cloud_link' ? "Cloud Link URL *" : "Description *"}
            required
            value={description}
            onChange={setDescription}
            placeholder={sharingMode === 'cloud_link'
              ? "https://example.com/your-config-link"
          ...
          {sharingMode === 'cloud_link' && (
            <p className="text-[10px] text-slate-400 mt-1 ml-1">Paste your hosted config URL. Users can tap to open or long-press to copy.</p>
          )}
        </div>
        ...
        <div>
          ...
          <div className={`grid gap-2 mb-2 ${sharingMode === 'cloud_link' ? 'grid-cols-4' : 'grid-cols-3'}`}>
            {expiryOpts.map((opt, idx) => (
              <button key={opt.label} type="button"
                onClick={() => { setActiveExpiryIdx(idx); if (sharingMode === 'downloadable') setShowCustom(false); }}
                className={`py-2 px-1 rounded-xl border text-center text-[10px] font-bold transition-all ${activeExpiryIdx === idx && (sharingMode === 'cloud_link' || !showCustom)
          ...
          {sharingMode === 'downloadable' && showCustom && (
          ...
          {sharingMode === 'downloadable' && !showCustom && EXPIRY_OPTIONS[expiryIdx].ms !== null && (
          ...
          {sharingMode === 'cloud_link' && CLOUD_EXPIRY_OPTIONS[cloudExpiryIdx].ms !== null && (
